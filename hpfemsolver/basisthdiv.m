function ph=basisthdiv(order,x,y);
if order==0
esize=3;
else
...
for i=1:3
ph(i,:)=curl(edges(i,1),:)*vph(edges(i,2))-curl(edges(i,2),:)*vph(edges(i,1));
end
...
ph(basno,1)=vj*duiy+ui*dvjy;
ph(basno,2)=-vj*duix-ui*dvjx;
end
end
end
...
basno=basno+1;
vj=l(3)*leg(l(3)-l(2)-l(1),j);
ph(basno,1)=(curl(edges(1,1),1)*vph(edges(1,2))-curl(edges(1,2),1)*vph(edges(1,1)))*vj;
ph(basno,2)=(curl(edges(1,1),2)*vph(edges(1,2))-curl(edges(1,2),2)*vph(edges(1,1)))*vj;
end
end

[m,n]=size(ph);
if m~=esize
  disp('error wrong number of basis functions basisthdiv')
end

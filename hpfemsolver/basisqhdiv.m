...
for m=1:4
ph(m,1:2)=1/2*lamplus(m)*csigdiff(m,1:2);
end
...
for p=1:order
for e=1:4
ph(4*p+e,2)=-dlegi(ep(e),p-1+2)*dep(e,1)*la(e)-legi(ep(e),p-1+2)*dla(e,1);
ph(4*p+e,1)=dlegi(ep(e),p-1+2)*dep(e,2)*la(e)+legi(ep(e),p-1+2)*dla(e,2);
end
end    
...
for j=0:1:order-1
basno=basno+1;
ph(basno,2)=legi(2*y-1,j+2)*dlegi(2*x-1,i+2)*2;
ph(basno,1)=-legi(2*x-1,i+2)*dlegi(2*y-1,j+2)*2;
end
end
...
for j=0:1:order-1
basno=basno+1;
ph(basno,2)=legi(2*y-1,j+2)*dlegi(2*x-1,i+2)*1;
ph(basno,1)=legi(2*x-1,i+2)*dlegi(2*y-1,j+2)*1;
end
end
...
for i=0:1:order-1
basno=basno+1;
ph(basno,2)=legi(2*y-1,i+2);
ph(basno,1)=0;
basno=basno+1;
ph(basno,2)=0;
ph(basno,1)=legi(2*x-1,i+2);
end
end

function [Meshsq,Meshst,splitq,orders,splitt]=split(ne,order,Meshq,nelemq,Mesht,nelemt,dir,glob,edges,coeff,coefft)

% this function splits each element into a predefined number of smaller elements
% based on the polynomial of the elements. This is in readiness for making
% a contour plot of the solution (which uses only linear interpolation).
% outputs:
% Meshsq: structure containing the split quadrilaterial mesh
% Meshst: structure containing the split triangular mesh
% splitq: connectivities for the splitting inside each quadrilaterial
% orders: parameter indicating the level of splitting
% splitt: same as splitt but for triangles

% fixed level of splitting (for speed)
orders=min(5,order+1);
...
%     create connectivities
   ...
   for j=1:(orders+3)^2
      [coordn(con(j),1),coordn(con(j),2)]=getcoord(xy,epetq(j,1),epetq(j,2));
   end
